...
public:
    ...
    virtual void discovery_change(Discovery * dis){
        vector<Service *> ls = this -> service(DIAL);
...
- (void) scan:(UIButton *)btn{
    m_is_scan = !m_is_scan;
    if (m_is_scan) {
        m_discovery -> scan( Discovery::DIAL );
...
#define CALL(exec) \
vector<Service*> ls = m_discovery->service( Discovery::DIAL ); \
for (int i = 0; i < ls.size(); i++) { \
...
- (void) start:(UIButton *)btn{
    ...
    vector<Service*> ls = m_discovery->service( Discovery::DIAL );
    for (int i = 0; i < ls.size(); i++) {
        Dial * mr = ls[i]->as_dial();
        if ( mr ) {
            Result res = mr -> launch("dev","123");
